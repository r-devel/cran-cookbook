...
`par()`, `options()` or `setwd()` all invisibly return their previous values and therefore those can be stored in variables using `<-`.
...
foo <- function(x){
  ...
  # your code which changes the options
...
This will reset the `par()`, `options()` and `setwd()`. The expression inside of `on.exit()` is executed when the function exits, including errors.  
...
If you change more than one option in the same function, example, vignette or demo, you can use `oldpar <- par(no.readonly = TRUE)` or `oldop <- options()` to reset all parameters at once. 
