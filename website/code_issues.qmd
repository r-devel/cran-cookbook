...
Omit the code line and rather use `suppressWarnings()` if really needed.
...
CRAN doesn't allow the use of negative `warn` options. This setting will turn off  all warning messages. 
`suppressWarnings()` also disable warnings, but only for the expression called within. 
