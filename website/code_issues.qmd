...
You are using more than 2 cores in your examples, vignettes, tests or during installation. 
...
CRAN checks run on big, multi-core computers, running many `R CMD check` processes in parallel. Every process is allowed to use a maximum of two CPU cores to exercise parallel code in examples and unit tests. When using more than 2 cores, your package will get a NOTE from CRAN checks, saying that, it took more CPU time than elapsed time.
...
The automatic tests will only produce a NOTE when more than 2 cores are used in examples, vignettes, tests or the installation.However, a possibility to set the amount of cores should be given in any function and the default is ideally set to less than 2 cores.

During installation, something that ran as part of the installation process started more than two child processes (or threads) at the same time. The solution is specific to the build system your package is using. Many modern build systems can use the `-j` flag which originated from `GNU Make`(from their [manual](https://devdocs.io/gnu_make/options-summary#-j%20[jobs]):)

> ‘-j [jobs]’\
Specifies the number of recipes (jobs) to run simultaneously. With no argument, `make` runs as many recipes simultaneously as possible. If there is more than one ‘-j’ option, the last one is effective.
