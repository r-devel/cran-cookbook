...
The `installed.packages()` function outputs a matrix with details of all packages which are installed in the specified libraries. This can be used to check if a specific package is installed. As mentioned in the help file of `installed.packages()`, it can be very slow under certain circumstances.
