...
The `installed.packages()` function outputs a matrix with details of all packages which are installed in the specified libraries. This can then be used to check if a specific package is installed. As mentioned in the help file of `installed.packages()`, it can be very slow under certain circumstances.
...
The help file lists different solutions and the respective, alternative functions.
...
Ideally, use `requireNamespace("pkg")` or `require("pkg")`, which return `FALSE` if a package is not available, and throw an error based on its return value. For more details on package installations in your code see [the corresponding recipe, (yet to come)]().
