...
Changing the seed per se is not forbidden. However, users should be able to control which seed to use. Code that changes the seed (e.g.: `set.seed(123)`) should therefore be avoided. A good solution, if you want to set a seed per default is to add an argument `seed` to your function and set it conditionally. 
