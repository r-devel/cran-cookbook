...
You are using more than 2 cores in your examples, vignettes or tests. 

## Solution

Make sure that you set the maximum number of cores used to 2. This is also important for installation, where the `-j 2` flag should be used. 
...
CRAN checks run on big, multi-core computers, running many `R CMD check` processes in parallel. Every process is allowed to use a maximum of two CPU cores to exercise parallel code in examples and unit tests. When using more than 2 cores a NOTE from CRAN checks is given, saying that during installation, it took more CPU time than elapsed time.
